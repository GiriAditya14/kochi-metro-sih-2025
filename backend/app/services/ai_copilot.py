...
For data queries, be specific with numbers and train IDs when possible."""
    ...
    def _fallback_response(self, prompt: str) -> str:
        ...
        if "plan explanation" in prompt.lower():
...
*Note: Enable Gemini AI (set GEMINI_API_KEY) for detailed explanations.*"""

        if "assignment" in prompt.lower():
            return "This assignment was determined by the multi-objective optimization algorithm based on fitness status, maintenance requirements, and operational priorities. Enable AI for detailed explanations."
...
*Enable Gemini AI for intelligent briefings.*"""
        
        return "AI features require GEMINI_API_KEY configuration. Please set the environment variable to enable intelligent responses."
    ...
    def chat(self, message: str, context: Dict = None) -> str:
        ...
        return loop.run_until_complete(self.answer_question(message, context))
    
    def explain_plan(self, plan_id: int) -> str:
        """Synchronous plan explanation"""
        ...
        return loop.run_until_complete(self.generate_plan_explanation(plan, assignment_dicts))
